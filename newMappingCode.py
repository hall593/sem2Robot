...
mazeMap = numpy.zeros(shape = (15,15)) # if numpy is on the pi, we'll use this insteas
...
def backUp(instance, orientation): # need to implement instance of changing map indicator, although could be a separate function
    ...
    if(checkTurns(orientation, coordinates[0], coordinates[1]) == 1):
      orientation = turnLeft(orientation)
    if(checkTurns(orientation, coordinates[0], coordinates[1]) == 2):
    ...
    while True:
        ...
        if(grovepi.ultrasonicRead(middleUltra) < 20):
            if (grovepi.ultrasonicRead(leftUltra) < 25 and grovepi.ultrasonicRead(rightUltra) < 25):
                orientation = backUp(1, orientation)
            ...
            print("IR? ", orientation, "is orientation")
            ...
            coordinates[0] = lastIX
            coordinates[1] = lastIY
            orientation = backUp(3, orientation)
