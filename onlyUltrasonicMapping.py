...
coordinates = [0, 0] # 0 index is x value, 1 index is y value, 2 index is orientation 
orientation = [0]

mazeMap = numpy.zeros(shape = (7,7)) # if numpy is on the pi, we'll use this instead
...
def move(orientation, coordinates):
    if(orientation[0] == 1): # oriented facing front
        coordinates[1] += 1
    elif(orientation[0] == 2): # oriented front facing left (relative to starting position)
        coordinates[0] += 1
    elif(orientation[0] == 3): # oriented front facing towards start
        coordinates[1] -= 1
    elif(orientation[0] == 4): # oriented front facing right (relative to starting position)
        coordinates[0] -= 1
